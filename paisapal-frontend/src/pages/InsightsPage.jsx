...
function InsightCard({ insight, onAction }) {
  ...
  return (
    <div className={`bg-surface-container backdrop-blur-md rounded-3xl border-l-4 ${typeConfig.borderColor} border border-outline-variant/30 p-6 hover:shadow-xl transition-all duration-300`}>
      ...
      <div className="space-y-4">
        ...
        {insight.details && insight.details.length > 0 && (
          <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4 border border-outline-variant/30">
          ...
          <div className="grid grid-cols-2 gap-4">
            {insight.impact.potential_savings && (
              <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800">
                <p className="text-xs font-medium text-on-surface-variant mb-1">Potential Savings</p>
            ...
            {insight.impact.time_frame && (
              <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                <p className="text-xs font-medium text-on-surface-variant mb-1">Time Frame</p>
            ...
            <div className="flex flex-wrap gap-2">
              {insight.recommended_actions.map((action, index) => (
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold bg-surface-container-highest text-on-surface rounded-lg hover:bg-secondary/20 dark:hover:bg-emerald-900/30 hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors"
                >
                  {action.title}
    ...
    <div className="bg-surface-container backdrop-blur-md rounded-3xl border border-outline-variant/30 p-6">
      ...
      <div className="space-y-3">
        {filteredData.length > 0 ? filteredData.map((forecast, index) => (
          <div key={index} className="flex items-center justify-between p-3 border border-outline-variant/30 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            <div>
            ...
            <div className="text-right">
              ...
              <p className={`text-sm font-medium ${
                ...
                forecast.trend === 'down' ? 'text-green-600 dark:text-green-400' : 'text-on-surface-variant'
              }`}>
                {forecast.trend === 'up' ? '↗' : forecast.trend === 'down' ? '↘' : '→'} 
                {Math.round((forecast.confidence || 0) * 100)}% confident
              </p>
            </div>
          </div>
        )) : (
          <p className="text-center text-on-surface-variant py-8">No forecast data available</p>
        )}
      </div>
    </div>
  )
      ...
      <div className="space-y-3">
        {tips.length > 0 ? tips.map((tip, index) => (
          <div key={index} className="flex items-start gap-3 p-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
            <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-primary to-secondary text-white rounded-full flex items-center justify-center text-sm font-bold">
              {index + 1}
            </div>
          ...
          <p className="text-center text-on-surface-variant py-8">Add more data to get personalized tips</p>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={refreshing}
            className="flex items-center gap-2 px-4 py-2 bg-surface-container-highest text-on-surface font-semibold rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-primary text-on-primary font-semibold rounded-lg hover:brightness-110 transition-all shadow-lg hover:shadow-xl"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { icon: Calculator, color: 'text-secondary', label: 'Total Insights', value: insights.length },
          ...
          return (
            <div key={index} className="bg-surface-container backdrop-blur-md rounded-3xl border border-outline-variant/30 p-6 hover:shadow-lg transition-all">
