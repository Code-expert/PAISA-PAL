    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800 py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-6 sm:space-y-8">
            ...
            <div>
              ...
              <div className="mt-3 space-y-1">
                <p className="text-xs text-on-surface-variant flex items-center">
                  <span className={`mr-2 ${password.length >= 8 ? 'text-secondary' : 'text-gray-400'}`}>
                    {password.length >= 8 ? '✓' : '○'}
                  </span>
                  At least 8 characters
                </p>
                <p className="text-xs text-on-surface-variant flex items-center">
                  <span className={`mr-2 ${/[A-Z]/.test(password) && /[a-z]/.test(password) ? 'text-secondary' : 'text-gray-400'}`}>
                    {/[A-Z]/.test(password) && /[a-z]/.test(password) ? '✓' : '○'}
                  </span>
                  Uppercase & lowercase letters
                </p>
                <p className="text-xs text-on-surface-variant flex items-center">
                  <span className={`mr-2 ${/\d/.test(password) ? 'text-secondary' : 'text-gray-400'}`}>
                    {/\d/.test(password) ? '✓' : '○'}
                  </span>
                  At least one number
                </p>
        ...
        <p className="text-center text-xs sm:text-sm text-on-surface-variant px-4">
          ...
          <Link to="/terms" className="text-secondary hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 font-medium transition-colors duration-200">
            Terms of Service
          </Link>{' '}
          and{' '}
          <Link to="/privacy" className="text-secondary hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 font-medium transition-colors duration-200">
            Privacy Policy
          </Link>
        </p>
      </div>
